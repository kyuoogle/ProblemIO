<template>
  <Card class="h-full hover:shadow-lg transition-duration-200 cursor-pointer" :class="{ 'opacity-75 grayscale-0': isExpired }" @click="handleClick">
    <template #header>
      <div v-if="challenge.challengeType === 'TIME_ATTACK'" class="bg-red-100 text-red-600 px-3 py-1 font-bold text-center">TIME ATTACK</div>
      <div v-else-if="challenge.challengeType === 'SURVIVAL'" class="bg-purple-100 text-purple-600 px-3 py-1 font-bold text-center">SURVIVAL</div>
      <div v-else class="bg-gray-100 text-gray-600 px-3 py-1 font-bold text-center">EVENT</div>
    </template>
